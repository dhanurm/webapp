...
public class EmployeeDaoImpl implements EmployeeDao {
	...
	@Override
	public EmployeeEntity fetchByuserName(String empname) {
		Session session = null;
		try {
			session = sfactory.openSession();

			Criteria crit = session.createCriteria(EmployeeEntity.class);
			Criteria add = crit.add(Restrictions.eq("name", empname));
			List<EmployeeEntity> list = (List<EmployeeEntity>) add.list();
